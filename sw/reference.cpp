...
{ // can easily genericize send_row_X further. TODO: do that
	...
	uint64_t lw_addr = C_START_ADDR + row * 0x10;
	uint64_t hw_addr = lw_addr + 0x8;

	// Partition the words into their respective rows
	for (int i = 0; i < DIM; i++) {
		uint64_t mask = 0x0FFFF; // mask per word

		uint64_t shift = (i * 16) % 64;

		send[i / 4] |= ((vals[i] & mask) << shift);

	}

	if (DEBUG)
		fprintf(stdout, "CWRITE: low word, high word, address %lx | %lx @%lx @%lx\n", send[0], send[1], lw_addr, hw_addr);

	afu.write(lw_addr, send[0]);
	afu.write(hw_addr, send[1]);
}
...
void unpack_from_C(uint16_t row, C_TYPE* vals, AFU& afu)
{
	...
	for (uint16_t i = 0; i < DIM; i++) {
		uint64_t mask = 0x0FFFF;

		uint64_t shift = (ind * 16) % 64;

		// Mask and store
		vals[ind] = ((wds[i / 4] & (mask << shift)) >> shift);
	}
}

void output_check(ptrdiff_t row, ptrdiff_t col)
{
	// Compare.
	int16_t got = output[row][col];
	int16_t expected = output_reference[row][col];
	fprintf(stdout, "row: %ld, col: %ld | got: %hx, expected %hx\n", row, col, got, expected);
	fflush(stdout);
	assert(got == expected);
}
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		fprintf(stdout, "[Write to C registers OK (at least the first 8x8...)]\n");

		// Now try it with the AFU.
		// Blocked multiply.

		// Call (x_o, y_o, z_o) = origin
		for (int y_o = 0; y_o < DIM_FULL; y_o += 8)
		{
			for (int x_o = 0; x_o < DIM_FULL; x_o += 8)
			{
				for (int z_o = 0; z_o < DIM_FULL; z_o += 8)
				{
					// Loading value of C to write
					fprintf(stdout, "Loading C into AFU...\n");
					for (ptrdiff_t c_rw = 0; c_rw < DIM; ++c_rw)
					{
						C_TYPE* origin_c = output[c_rw + y_o] + x_o;
						send_row_C(c_rw, origin_c, afu);
					}
					...
					{
						AB_TYPE* origin_a = A_vals[a_r + y_o] + z_o;
						send_row_A(a_r, origin_a, afu);
					}
					...
					{
						AB_TYPE* origin_b = B_vals[b_r + z_o] + x_o;
						send_row_B(b_r, origin_b, afu);
					}

					// Calculate
					fprintf(stdout, "Performing Calculation...\n");
					afu.write(0x0500, 0); // Starts the calculation
					...
					{
						unpack_from_C(c_r, output[c_r + y_o] + x_o, afu);
					}
				}
			}
		}
